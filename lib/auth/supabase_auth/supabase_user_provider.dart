...
class MyGoalsSupabaseUser extends BaseAuthUser {
  MyGoalsSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> myGoalsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MyGoalsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
